...
int main() {
	const unsigned int lookupRPM[] = {700, 1200, 1600, 2100, 2500, 2900, 3400, 3800, 4200, 4600, 5100, 5500}; // RPM
	const unsigned int lookupLoad[] = {25, 32, 38, 46, 52, 60, 66, 74, 80, 88, 94, 101}; // % ABS Load
...
//	   700,  1200, 1600, 2100, 2500, 2900, 3400, 3800, 4200, 4600, 5100, 5500 RPM
	...
	if (rpm > lookupRPM[XLength- 1]) {
			  ...
			  rpmIndex = XLength - 1;
	} else while (rpmIndex < XLength && rpm >= lookupRPM[rpmIndex+1]) rpmIndex++;

/*
	 int loadIndex = 0;
	 while (loadIndex < YLength && load >= lookupLoad[loadIndex+1]) loadIndex++;

	 int rpmIndex = 0;
	 while (rpmIndex < XLength && rpm >= lookupRPM[rpmIndex+1]) rpmIndex++;
*/	                                                                                                                               
...
private:
	const unsigned int lookupRPM[] = {700, 1200, 1600, 2100, 2500, 2900, 3400, 3800, 4200, 4600, 5100, 5500}; // RPM	  
	const unsigned int lookupLoad[] = {25, 32, 38, 46, 52, 60, 66, 74, 80, 88, 94, 101}; // % ABS Load              	  
	const unsigned int XLength = sizeof(lookupRPM)/sizeof(int);                                                                        	  
	const unsigned int YLength = sizeof(lookupLoad)/sizeof(int);
...
//	   700,  1200, 1600, 2100, 2500, 2900, 3400, 3800, 4200, 4600, 5100, 5500 RPM
	...
	public:
		void setLookup(Vec2D _lookup) {
			...
			this->table.y = YLength - _lookup.y - 1;
		}
		...
		TablePosition getNextX() {
			...
			if (temp.x >= XLength) temp.x = XLength - 1;
