...
static float FuelInjector::calculateOpenFactor(int rpm, float airFlow, float airFuelRatio) {
  ...
  else if (rpm > 8000) openFactor = .012*airFlow*3.0; // .01 - .012 ; If rpm is extremely high, noise is present and engine is starting.
  //airFuelRatio = airGramsPerMinute / (Constants::Engine::injectorGramsPerMinute * Constants::Engine::numberOfCylinders * openFactor);
  else openFactor = (airGramsPerMinute / (Constants::Engine::injectorGramsPerMinute * Constants::Engine::numberOfCylinders * airFuelRatio))*3.0*3.0;
